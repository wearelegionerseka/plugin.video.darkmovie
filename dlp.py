...
def get_url(**kwargs):
    ...
    #logga(f'Sono in def get_url  passo URL -->{url}')
    return url
...
def show_years(which):
    ...
    for a in range(year, 1900, -1):
        ...
        #logga(f'Sono in def show_years url-->{url}')
        is_folder = True
        xbmcplugin.addDirectoryItem(_handle, url, list_item, is_folder)
...
def search_movie(
    ...
    for result in results:
        ...
        #logga(f'Sono in def search_movie url-->{url}')
        is_folder = True
        xbmcplugin.addDirectoryItem(_handle, url, list_item, is_folder)
        pDialog.update(progress, message)
        times += 1
...
def list_mirros_movie(title, en_title, metadata_art, metadata_movie, metadata_cast):
    ...
    for a in sites_film:
        ...
        progress = 100 * times // l_results
        link = find_link(a, title)
        #logga(f' eseguo find_link con {a} e {title}')
        #logga(f'link-->{link}')
        if not link:
            link = find_link(a, en_title)
            #logga(f' eseguo find_link (inglese) con {a} e {en_title}')
            #logga(f'link-->{link}')
        if not link:
            pDialog.update(progress, new_string)
            times += 1
    ...
    for mirror in mirrors:
        ...
        list_item.setInfo('video', metadata_movie)
        list_item.setProperty('IsPlayable', 'true')
        #logga(f'def find_link metadata_movie-->{metadata_movie}')
        
        #logga('mirror link-->'+str(mirror['link']))
        url = get_url(
            action = 'play', video = mirror['link'],
            mirror =  mirror['mirror'], domain = mirror['domain'], title = title
        )
...
def router(paramstring):
    ...
    if params:
        if params['action'] == settings.menu_items[0][0]:
            ...
            try:
                cerco_en_title=params['en_title']
